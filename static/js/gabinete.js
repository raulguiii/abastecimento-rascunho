document.addEventListener("DOMContentLoaded", function () {
    let dadosCarregadosGabinete = false;

    function renderTabelaGabinete(dados) {
        const tabela = document.getElementById("tabelaGABINETE");
        tabela.innerHTML = "";
    ...
    document.querySelector('[data-tab="gabineteHist"]').addEventListener("click", function () {
        if (!dadosCarregadosGabinete) {
            fetch("/abastecimentoGabineteHist")
                .then(response => response.json())
                .then(data => {
                    renderTabelaGabinete(data);
                    dadosCarregadosGabinete = true;
                })
