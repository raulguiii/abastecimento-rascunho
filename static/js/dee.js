document.addEventListener("DOMContentLoaded", function () {
    let dadosCarregadosDEE = false;

    function renderTabelaDEE(dados) {
        const tabela = document.getElementById("tabelaAbastecimentosDEE");
        tabela.innerHTML = "";
    ...
    document.querySelector('[data-tab="deeHist"]').addEventListener("click", function () {
        if (!dadosCarregadosDEE) {
            fetch("/abastecimentoDEEHist")
                .then(response => response.json())
                .then(data => {
                    renderTabelaDEE(data);
                    dadosCarregadosDEE = true;
                })
