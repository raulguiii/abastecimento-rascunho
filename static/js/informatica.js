document.addEventListener("DOMContentLoaded", function () {
    let dadosCarregadosInformatica = false;

    function renderTabelaInformatica(dados) {
        const tabela = document.getElementById("tabelaINFORMATICA");
        tabela.innerHTML = "";
    ...
    document.querySelector('[data-tab="informaticaHist"]').addEventListener("click", function () {
        if (!dadosCarregadosInformatica) {
            fetch("/abastecimentoInformaticaHist")
                .then(response => response.json())
                .then(data => {
                    renderTabelaInformatica(data);
                    dadosCarregadosInformatica = true;
                })
